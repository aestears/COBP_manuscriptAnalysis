...
IPMs_CC_HH_bootCI <- lapply(IPMs_CC_HH_bootCI_lambdas,
                          FUN = function(x)
                            c(log(c((mean(x[1:1000]) - 1.96*sd(x[1:1000])/sqrt(100)),
                                  (mean(x[1:1000]) + 1.96*sd(x[1:1000])/sqrt(100)))),
                              "n" = sum(!is.na(x)))
)
IPMs_CC_HH_bootCI <- lapply(IPMs_CC_HH_bootCI, 
                            FUN = as.numeric)
...
for (i in c(1:6)) {
  ...
  for(j in 1:1000) {
    ...
    # make subdatasets for vital rate models
    survDat_now <- boot_data[boot_data$flowering==0 | is.na(boot_data$flowering),]
    seedDat_now <- boot_data[boot_data$flowering==1,]
    recD_now <- dat_all[dat_all$seedling == 1 & dat_all$Year == 2019,]
    ...
    tryCatch({
      ...
      lambda_now <- base::eigen(mat)$values[1]
      paramsSite_now <-  as.data.frame(c(paramCont, "subPop" =unique(dat_all$Site)[i]))
    },  
    error=function(e){
      lambda_now <<- NA
      paramsSite_now <<- c(as.vector(rep(NA, length.out = 16), mode = "double"), "subPop" = unique(dat_all$Site)[i])
    }
       )

    
    # save the lambda
    siteDI_lambdas[j] <- lambda_now
     if (j == 1000) {
      IPMs_II_NN_bootCI_lambdas[[i]] <- siteDI_lambdas
    }
    # save the parameters
    if (j == 1 & i == 1){
      IPMs_II_NN_bootCI_params <- paramsSite_now
    } else {
      IPMs_II_NN_bootCI_params <- rbind(IPMs_II_NN_bootCI_params, paramsSite_now)
    }
  }
}
IPMs_II_NN_bootCI <- lapply(IPMs_II_NN_bootCI_lambdas,
                           FUN = function(x)
...
IPMs_II_NN_bootCI <- lapply(IPMs_II_NN_bootCI, 
       FUN = as.numeric)

names(IPMs_II_NN_bootCI) <- unique(dat_all$Site)

lapply(IPMs_II_NN_bootCI_lambdas, 
       FUN = function(x) log(mean(x)))

means <- lapply(IPMs_II_NN_bootCI_lambdas, FUN = function(x) log(mean(x[1:1000], na.rm = TRUE)))
names(means) <- unique(dat_all$Site)
