...
## model parameters stored in the det_DI_mods list (in the COBP_IPM_02_VitalRateModels.R script)

### run IPMs inside a for-loop
IPM_C_H_list <- vector(mode = "list", length = length(sites))
...
for (i in 1:length(sites)) {
  ...
  temp_ipm <- init_ipm(sim_gen = "general", # make a general IPM
    ...
    define_kernel(
      ...
      s.             = inv_logit(lin.pred =  (s_int + s_slope * ht_1)),
      p_b.          = inv_logit(lin.pred = (p_b_int+ p_b_slope*ht_1 + p_b_slope_2 * (ht_1)^2)),
      data_list     = data_list,
      states        = list(c('ht')),
      uses_par_sets = FALSE,
      evict_cor     = TRUE,
  ...
  IPM_C_H_list[[i]] <- assign(paste0(site_now,"__det_DI_ipm"), value = temp_ipm)
...
IPM_I_N_list <-  vector(mode = 'list', length = length(sites))
...
for (i in 1:length(sites)) {
  ...
  temp_ipm <- init_ipm(sim_gen = "general", # make a general IPM
    ...
    define_kernel(
      ...
      s.             = inv_logit(lin.pred = (s_int + s_slope * ht_1 + s_dd * sum(n_ht_t))),
      p_b.          = inv_logit(lin.pred= (p_b_int + p_b_slope * ht_1 + p_b_slope_2 * (I(ht_1)^2))),
      data_list     = data_list,
      states        = list(c('ht')),
      uses_par_sets = FALSE,
      evict_cor     = TRUE,
  ...
  ## rename the ipm object to have the name of site and store it
  IPM_I_N_list[[i]] <- assign(paste0(site_now,"__det_DD_ipm"), value = temp_ipm)
...
for (i in 1:length(sites)) {
  ...
  temp_ipm <- init_ipm(sim_gen = "general", # make a general IPM
    ...
    define_kernel(
      ...
      s.             = inv_logit(lin.pred = (s_int + s_slope * ht_1 + s_soilM * soilM + s_soilT * soilT + s_dd * sum(n_ht_t))), # 0.62
      p_b.          = inv_logit(lin.pred = (p_b_int + p_b_slope * ht_1 +  p_b_slope_2 * I(  ht_1)^2 + p_b_soilM * soilM + p_b_temp * temp + p_b_precip * precip)), # 0.0029
      data_list     = data_list,
      states        = list(c('ht')),
      uses_par_sets = FALSE,
      evict_cor     = TRUE,
    ...
    define_kernel(
      ...
      family        = "CD",
      goSdlng.      = goSdlng,
      p_b.          = inv_logit(lin.pred = (p_b_int + p_b_slope * ht_1 +  p_b_slope_2 *I(ht_1)^2 + p_b_soilM * soilM + p_b_temp * temp + p_b_precip * precip)),
...
for(i in 1:200) {
  ...
  sample_disc <- seq(1, nrow(discDat), by = 1)
  
  boot_disc_ind <- sample(sample_disc, size = nrow(discDat), replace = TRUE)
  
  boot_disc <- discDat[boot_disc_ind,]
...
ipmr::lambda(contSeedlings_IPM_dd)
## check for eviction from the model
#To check for eviction, we plot the survival model and the column sums of the survival/growth (P) matrix. Eviction occurs when the column sums are lower than the survival models suggests that they should be.
# define the x-axis values
meshpts <- seq(from = (min(dat_all$log_LL_t, na.rm = TRUE) * .8), to = (max(dat_all$log_LL_t, na.rm = TRUE) * 1.2) , length.out = 500)
...
# #### Determinsitic, DI IPM for Base data ####
# data_list <- list(
...
#### Deterministic, density-independent IPM for all data -- w/ determinsitic seedlings! ####
...
det_ipm <- init_ipm(sim_gen = "general", # make a general IPM
  ...
  define_kernel(
    ...
    p_b.          = inv_logit(p_b_int + p_b_slope * ht_1 + p_b_slope_2 * I(ht_1^2)),
    data_list     = data_list,
    states        = list(c('ht')),
    uses_par_sets = FALSE,
    evict_cor     = TRUE,
...
#     s_plot        = inv_logit(lin.pred= (s_int + s_slope*ht_1 + 
#                                            s_soilM * soilM + s_soilT * soilT + s_temp * temp + # env covariates
#                                            s_r_plot # ranef of plot
#     )),
#     p_b_plot      = inv_logit(lin.pred= (p_b_int + p_b_slope * ht_1 + p_b_slope_2 * I((ht_1)^2) + 
...
#                                            s_r_plot # ranef of plot
#     )),
#     p_b_plot      = inv_logit(lin.pred= (p_b_int + p_b_slope * ht_1 + p_b_slope_2 * ( ht_1)^2 + 
