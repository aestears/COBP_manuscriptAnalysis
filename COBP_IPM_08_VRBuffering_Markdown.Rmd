...
ggplot(bigDF_minSbSd) +
   geom_smooth(aes(y = mean_corrSD, x = mean_VSS), method = "lm", col = "darkgrey", se = FALSE) +
  #geom_point(aes(x = mean_VSS, y = mean_corrSD)) + 
  geom_pointrange(aes(x = mean_VSS, y = mean_corrSD, xmin = (mean_VSS - sd_VSS), xmax = (mean_VSS + sd_VSS))) +
  geom_pointrange(aes(x = mean_VSS, y = mean_corrSD, ymin = mean_corrSD - sd_corrSD, ymax = mean_corrSD + sd_corrSD)) + 
  xlab("meanVSS") +
  ylab("mean corrected SD") + 
  geom_text(aes(x = 1.5, y = 2.5, label = paste0("corr=", round(cor_minSbSd$estimate, 2),"; P-val=", round(cor_minSbSd$p.value, 3)))) +
  theme_classic()
...
ggplot(bigDF_maxSbSd) +
  ...
  xlab("meanVSS") +
  ylab("mean corrected SD") + 
  geom_text(aes(x = 1.5, y = 2.5, label = paste0("corr=", round(cor_maxSbSd$estimate, 2),"; P-val=", round(cor_maxSbSd$p.value, 3)))) +
  theme_classic()
  
  
cor_maxSbSd <- cor.test(bigDF_maxSbSd$mean_VSS, bigDF_maxSbSd$mean_corrSD)
# correlation: cor = .83, P-value = .006
plot(bigDF_maxSbSd$mean_VSS, bigDF_maxSbSd$mean_corrSD, pch = 16,
     xlab = c("mean VSS"), ylab = c("mean corrected SD"))
abline(lm( bigDF_maxSbSd$mean_corrSD ~ bigDF_maxSbSd$mean_VSS))
text(x = .4, y = .5, labels = paste0("corr=", round(cor_maxSbSd$estimate, 2),"; P-val=", round(cor_maxSbSd$p.value, 2)))
```
