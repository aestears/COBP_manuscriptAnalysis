...
corr.growth.sd <-
  apply(car::logit(simplify2array(growth),adjust = .001), 1, sd)
```
...
corr.seedlingSize.sd <-
  apply(car::logit(simplify2array(seedlingSize),adjust = .001), 1, sd)
```
...
```{r}
corr.outSB.sd <- sd(car::logit(seq(from = .01, to = .99, length.out = 20)))
corr.staySB.sd <- sd(car::logit(seq(from = .01, to = .99, length.out = 20)))
corr.goCont.sd <- sd(car::logit(seq(from = .01, to = .99, length.out = 20)))
corr.goSB.sd <- sd(car::logit(seq(from = .01, to = .99, length.out = 20)))
```
...
# get and save the perturbed vital rate function
G_perturb <- h * GR.fun_pert(z = meshp, zz = meshp, paramCont = paramCont )
```
...
VSS.growth <- as.numeric(sens_Growth * ((growth.mean*(1-growth.mean))/lambda_OG))
...
SDS.fun_pert <- function(zz, paramCont){
  rec_mu <- paramCont[["recruitDist"]][1] + perturbVal
  rec_sd <- paramCont[["recruitDist"]][2]
  return(dnorm(zz, mean = rec_mu, sd = rec_sd))
}
...
```{r}
# VSS on SeedlingSize 
VSS.SeedlingSize <- as.numeric(sens_SeedlingSize * ((seedlingSize.mean*(1-seedlingSize.mean))/lambda_OG))
...
Do this for each vital rate size class

```{r}
# put all the vital rates into one data.frame
bigDF <- data.frame("VitalRate" = c(rep("surv", 500), rep("seedProd", 500), rep("flowerProb", 500), rep("growth", 500), rep("seedlingSize", 500)),
                    "sizeClass" = rep(1:500, times = 5),
                    "VSS" = c(VSS.surv, VSS.seeds, VSS.flwr, VSS.growth, VSS.SeedlingSize), 
                    "correctedSD" = c(corr.surv.sd, corr.seedProd.sd, corr.flowering.sd, corr.growth.sd, corr.seedlingSize.sd)
                    )

for (i in 1:500) {
  # calculate the correlation
  cor_temp <- cor.test(c(bigDF[bigDF$sizeClass == i, "VSS"], VSS.outSB, VSS.staySB, VSS.goCont, VSS.goSB), 
                       c(bigDF[bigDF$sizeClass == i, "correctedSD"], corr.outSB.sd, corr.staySB.sd, corr.goCont.sd, corr.goSB.sd),
                       method = "pearson")
  
  
  ggplot(data = bigDF[bigDF$sizeClass == i,]) +
  ...
  if (i == 1) {
    correlations <- data.frame("sizeClass" = i, "correlation" = cor_temp$estimate, "corr_pVal" = cor_temp$p.value, "corr_confIntLow" = cor_temp$conf.int[1], "corr_confIntHigh" = cor_temp$conf.int[2])
  } else {
   correlations <-  rbind(correlations, data.frame("sizeClass" = i, "correlation" = cor_temp$estimate, "corr_pVal" = cor_temp$p.value, "corr_confIntLow" = cor_temp$conf.int[1], "corr_confIntHigh" = cor_temp$conf.int[2]))
  }
}

# visualize the correlation coefficients for each size class 
ggplot(data = correlations) +
  geom_ribbon(aes(ymin = corr_confIntLow, ymax = corr_confIntHigh, x = 1:500), fill = "lightgrey", alpha = .5) +
